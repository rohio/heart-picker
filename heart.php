...
if($array_user["favourites_count"] < $display_num){
	...
	foreach($array as $key => $value){
		...
		// var_dump($value);
		// 埋め込みtweetの形で表示
		$html .= '<blockquote class="twitter-tweet" data-lang="ja"><p lang="ja" dir="ltr">' . $value["text"] . '</p>&mdash; ' . $value["user"]["name"] . '(@' . $value["user"]["screen_name"] . ') <a href="https://twitter.com/' . $value["user"]["screen_name"] . '/status/' . $value["id_str"] . '?ref_src=twsrc%5Etfw">' . $value["created_at"] . '</a></blockquote>';
		$display_count++;
